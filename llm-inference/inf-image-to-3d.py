...
APP_NAME = "image-to-3d"
CLASS_NAME = "ImageTo3D"
DEFAULT_IMAGE = Path(__file__).resolve().parent / "img.png"
DEFAULT_OUTPUT = Path(__file__).with_name("image-to-3d-output.glb")
MIN_GLB_BYTES = 1_000
...
def run_pipeline(
    ...
    print("Calling remote ImageTo3D pipeline class on Modal...")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run image-to-3D pipeline (background removal + 3D gen) against deployed Modal app."
    )
