...
APP_NAME = "trellis-2-4b-fast"
CLASS_NAME = "Trellis2FastModel"
DEFAULT_IMAGE = Path(__file__).resolve().parents[1] / "llm-hosting" / "demo.png"
DEFAULT_OUTPUT = Path(__file__).with_name("trellis-2-4b-fast-output.glb")
MIN_GLB_BYTES = 1_000
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fast image-to-3D inference against the deployed TRELLIS.2 Modal app."
    )
    ...
    parser.add_argument(
        "--bf16",
        action="store_true",
        help="Enable experimental BF16 autocast. Disabled by default because TRELLIS.2 sparse convolutions can reject mixed dtypes.",
    )
...
if __name__ == "__main__":
    ...
    print(f"Saved fast TRELLIS.2 GLB to {output} ({os.path.getsize(output)} bytes)")
